...
(function ($) {
  var Attendance = function (elem) {
    ...
    this.eSubmitButton = $(elem.submitButton)
  ...
  Attendance.prototype.getClassCourseSuccess = function (res) {
    if (res.code === OK) {
      ...
      if (res.data.length <= 0) {
        ...
        $('.course:not(.warning)').on('click', $.proxy(this.getAttendance, this))
      }
    ...
    if (res.code === OK) {
      ...
      if (res.data.students.length === 0) {
        ...
        for (var i = 0; i < res.data.students.length; i++) {
          tr += '<tr><td>' + res.data.students[i].class + '</td><td>' + res.data.students[i].studentId + '</td><td>' + res.data.students[i].name + '</td><td>' + '<select class="form-control">'
          for (var j = 1; j <= 6; j++) {
            if (j === 1) {
      ...
      this.eSubmitButton.on('click', $.proxy(this.postAttendance, this))
  ...
  Attendance.prototype.postAttendance = function () {
    this.eSubmitButton.text('提交中')
    this.eSubmitButton.addClass('disabled')
    var data = 'userId=' + this.userId + '&attendanceId=' + this.attendanceId
    var trData = this.eDataTable.find('tr')
    var arr = []

    for (var i = 1; i < trData.length; i++) {
      if (trData.eq(i).find('td:eq(3)>select:eq(0)').val() === '1') {
        continue
      }
      var temp = {}
      temp.attendanceId = this.attendanceId
      temp.studentId = trData.eq(i).find('td:eq(1)').text()
      temp.stateId = trData.eq(i).find('td:eq(3)>select:eq(0)').val()
      arr.push(temp)
    }
    data += '&attendance=' + window.JSON.stringify(arr)
    $.ajax({
      url: 'api/postAttendance',
      type: 'POST',
      data: data,
      success: $.proxy(this.postAttendanceSuccess, this),
      error: $.proxy(this.postAttendanceError, this),
      timeout: 10000,
      xhrFields: {
        withCredentials: true
      }
  ...
  Attendance.prototype.postAttendanceSuccess = function (res) {
    this.eSubmitButton.text('提交')
    this.eSubmitButton.removeClass('disabled')
    if (res.code === OK) {
  ...
  Attendance.prototype.postAttendanceError = function () {
    this.eSubmitButton.text('提交')
    this.eSubmitButton.removeClass('disabled')
    this.eSubmitAlert.text('网络错误,请刷新重试')
    this.eSubmitAlert.fadeIn()
  }
